...
# Molecule container names
playbook_containers = ['debian', 'ubuntu', 'centos', 'fedora', 'arch']
...
os_install_cmds = {
    ...
    'rhel': 'dnf install -y python3',
    'fedora': 'dnf install -y python3',
    'centos': 'dnf install -y python3 python3-dnf',
...
for container in running:
    ...
    os_stream = client.exec_start(
        client.exec_create(cid, ['/bin/sh', '-c', 'grep "^ID=" /etc/os-release 2>/dev/null | cut -d= -f2 | tr -d \'"\''], 
    ...
    try:
        install_stream = client.exec_start(
            client.exec_create(cid, ['/bin/sh', '-c', cmd], stdout=True, stderr=True), 
            stream=1)
...
#    ["ansible-playbook", "-i", "/home/a/.ansible/tmp/molecule.9ITD.default/inventory", "/home/a/netology/modules/Git/devops-netology/HW/Configuration_management/Ansible_roles/main_task/molecule/default/site.yml"],
#    cwd="/home/a/netology/modules/Git/devops-netology/HW/Configuration_management/Ansible_roles/main_task/molecule/default")
#
